...
# find_cann_package(ASC)时搜索路径插入asc-devkit源码目录
function(__cann_insert_asc_path_in_find_package_argn outvar asc_path)
    set(ASC_ARGN)
    set(HAS_PATHS FALSE)
    foreach(arg IN LISTS ARGN)
        if(arg STREQUAL "PATHS")
            set(HAS_PATHS TRUE)
            list(APPEND ASC_ARGN "${arg}" "${asc_path}")
        else()
            list(APPEND ASC_ARGN "${arg}")
        endif()
    endforeach()
    if(NOT HAS_PATHS)
        list(APPEND ASC_ARGN PATHS "${asc_path}")
    endif()
    set(${outvar} "${ASC_ARGN}" PARENT_SCOPE)
endfunction()
...
macro(find_cann_package)
    if(TOPLEVEL_PROJECT OR "${ARGV0}" IN_LIST CANN_BINARY_COMPONENTS OR CANN_BINARY_COMPONENTS_ALL)
        find_package(${ARGN})
    elseif("${ARGV0}" STREQUAL "ASC" OR "${ARGV0}" STREQUAL "asc")
        __cann_insert_asc_path_in_find_package_argn(_ASC_ARGN ${CUSTOM_ASC_CMAKE_PATH} ${ARGN})
        find_package(${_ASC_ARGN})
        unset(_ASC_ARGN)
    endif()
endmacro()
