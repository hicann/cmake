...
do_copy_files() {
    ...
    # Shell 脚本（可执行）
    find ./ -type f \( -name "*.py" -o -name "*.sh" \) -exec chmod "${exec_mod}" {} + 2>/dev/null
...
set_install_permissions() {
    ...
    if [ -d "${install_path_full}/python/site-packages" ]; then
        ...
        find "${install_path_full}/python/site-packages" -type f \( -name "*.py" -o -name "*.sh" \) -exec chmod "${exec_mod}" {} + 2>/dev/null
    fi
